...
import { CATEGORY_INFO, STATUS_INFO, DEPARTMENT_INFO, type CivicIssue, type IssueStatus } from "@/lib/civic-data";
...
const AdminDashboard = () => {
  ...
  const { data: issues = [], isLoading } = useQuery({ queryKey: ["admin-issues"], queryFn: fetchIssues });
  ...
  const updateStatusMutation = useMutation({
    mutationFn: async ({ id, status }: { id: string; status: IssueStatus }) => {
      const { error } = await supabase.from("issues").update({ status, updated_at: new Date().toISOString() } as any).eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["admin-issues"] }); toast.success("Status updated"); },
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["admin-issues"] }); toast.success("Report removed"); },
    onError: (e: any) => toast.error(e.message),
  });
    ...
    return (
      <div className="min-h-screen pt-24 pb-12 flex items-center justify-center">
        <div className="glass-card rounded-2xl p-10 text-center max-w-md">
          ...
          <p className="text-muted-foreground mb-6">You don't have admin privileges. Contact the administrator.</p>
            ...
            <div className="glass-card rounded-xl p-4 mb-6 flex flex-wrap gap-3 items-center">
              ...
              <Select value={categoryFilter} onValueChange={(v) => setCategoryFilter(v as any)}>
                ...
                <SelectContent>
                  ...
                  {Object.entries(CATEGORY_INFO).map(([k, v]) => (<SelectItem key={k} value={k}>{v.icon} {v.label}</SelectItem>))}
                </SelectContent>
              </Select>
              <Select value={statusFilter} onValueChange={(v) => setStatusFilter(v as any)}>
                ...
                <SelectContent>
                  ...
                  {Object.entries(STATUS_INFO).map(([k, v]) => (<SelectItem key={k} value={k}>{v.label}</SelectItem>))}
                </SelectContent>
              </Select>
            </div>
                    ...
                    <tbody>
                      {filtered.map((issue) => (
                        <tr key={issue.id} className="border-b border-border/50 hover:bg-muted/30 transition-colors">
                          ...
                          <td className="py-3 px-4">
                            <Select value={issue.status} onValueChange={(v) => updateStatusMutation.mutate({ id: issue.id, status: v as IssueStatus })}>
